    ...
    JsCalendar.prototype._create = function () {
        ...
        for (i = 0; i < 7; i++) {
            ...
            name = this._options.dayFormat.replace(/(DAY|day|DDD|ddd|DD|dd|D)/g, function (key) {
                return lang.dayStringParser('DDD', dayIndex);
            });
...
var setCalendar = function () {
    ...
    window.addEventListener('load', function () {
        ...
        //jsCalendar.autoFind();
        //test();
        var elem = document.getElementById("my-calendar");
        var myCal = jsCalendar.new(elem);
        myCal.onDateClick(function (event, date) {

            // On day click
            var curMonth = date.getMonth() + 1;
            var curDate = ((date.getDate() > 9) ? date.getDate() : "0" + date.getDate()) + "/" + (curMonth < 9 ? "0" + curMonth : curMonth) + "/" + date.getFullYear();
            //curDate = myCal._parseDate(curDate.toString());
